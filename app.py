...
st.markdown("<h1 style='color:#ff4b4b;'>🔥 FitPulse: Personalized Fitness Planner</h1>", unsafe_allow_html=True)
...
# --- Tabs Layout ---
tab1, tab2, tab3, tab4 = st.tabs(["🏠 Dashboard", "🏋️ Workouts", "🥗 Meals", "📊 Progress Tracker"])

# --- Dashboard Tab ---
with tab1:
    if generate:
        st.subheader("🎯 Personalized FitPlan")
        ...
        with st.expander("💪 Gym Workouts"):
            if gym_df.empty:
                ...
                for _, row in gym_df.iterrows():
                    st.markdown(f"<span style='color:#90ee90;'>✅ {row['Exercise']} — <b>{int(row['Estimated Burn'])} kcal/hr</b></span>", unsafe_allow_html=True)
        ...
        for day, info in weekly_meals.items():
            with st.expander(f"📅 {day} — {info['total']} kcal"):
                for meal in info["meals"]:
                    st.markdown(f"🍽️ <b>{meal['dish']}</b> — {meal['calories']} kcal at _{meal['location']}_", unsafe_allow_html=True)
